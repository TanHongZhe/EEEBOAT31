...
int magnet_total[3] = {0, 0, 0};
int magnet_average[3] = {0, 0, 0};
const int inputPinmag[3] = {A0, A1, A2};  // Note: You may need to change these pins for your board
double outputv[3] = {0, 0, 0};
...
// Helper function to read magnetic sensor
int ReadMagnetic(int i) {
  magnet_total[i] = 0;  // Reset total before reading
  for (int l = 0; l < numReadings; l++) {
    magnet_total[i] = magnet_total[i] + analogRead(inputPinmag[i]);
  }
  return magnet_total[i];
}

// Main magnetic detection function
String get_magnetic() {
  // Read all 3 magnetic sensors
  for (int i = 0; i < 3; i++) {
    magnet_average[i] = ReadMagnetic(i) / numReadings;
  }
  
  // Convert to voltage and check polarity for each sensor
  for (int i = 0; i < 3; i++) {
    outputv[i] = (magnet_average[i] * 3.3) / 1023.0;
    
    // Check if pin is floating (no sensor connected)
    // Floating pins usually read very low or very high values
    if (magnet_average[i] < 50 || magnet_average[i] > 950) {
      continue; // Skip this sensor - likely not connected
    }
    ...
    else if (outputv[i] < 2.4) {
      // North pole detected  
      return "Up";
    }
    // If voltage is between 2.4-2.7V, continue to next sensor
  }
  
  // No magnetic field detected on any sensor OR no sensors connected
  return "None";
}

String get_species(String infrared, String radio, String magnetic) {
    // Convert string inputs to integers for comparison
    int infrared_val = infrared.toInt();
    int radio_val = radio.toInt();
    
    // Wibbo: Infrared=457Hz, Radio=N/A, Magnetic=Down
    if (infrared_val >= 447 && infrared_val <= 467 && magnetic == "Down") {
        return "Wibbo";
    }
    
    // Gribbit: Infrared=N/A, Radio=100Hz, Magnetic=Down  
    if (radio_val >= 75 && radio_val <= 125 && magnetic == "Down") {
        return "Gribbit";
    }
    
    // Snorkle: Infrared=293Hz, Radio=N/A, Magnetic=Up
    if (infrared_val >= 283 && infrared_val <= 303 && magnetic == "Up") {
        return "Snorkle";
    }
    
    // Zapple: Infrared=N/A, Radio=150Hz, Magnetic=Up
    if (radio_val >= 125 && radio_val <= 175 && magnetic == "Up") {
        return "Zapple";
    }
...
String get_radio() {
  ...
  while (!radioDoneAveraging) {
    if (millis() - startTime > 1000) {
      ...
      return "0";  // ← FIXED: Return "0" for timeout
    }
  }
  ...
  if (avgPeriod > 0) {
    ...
    if (frequency >= minFrequency && frequency <= maxFrequency) {
      ...
      return String((int)frequency);  // ← NEW: Return just the number
    } else {
      radio_status = "out of range: " + String(frequency, 2) + " Hz";
      return "0";  // ← NEW: Return "0" for out of range
    }
  }

  radio_status = "no signal";
  return "0";  // ← NEW: Return "0" for no signal
}
...
void displayStatus() {
  ...
  Serial.println("Magnetic: " + get_magnetic());  // ← ADD THIS LINE
  Serial.println("Species: " + speciesValue);
  Serial.println("-------------------");
}
...
void loop() {
  ...
  unsigned long infraredFreq = get_infrared();
  String radioFreq = "0";
  String magneticField = get_magnetic();

  speciesValue = get_species(String(infraredFreq), radioFreq, magneticField);
...
void sendJoystickPage(WiFiClient client) {
  ...
  client.println(F("      document.getElementById('nameValue').textContent = data.name;"));
  client.println(F("      document.getElementById('speciesValue').textContent = data.species;"));
  client.println(F("      document.getElementById('infraredValue').textContent = data.infrared;"));
  client.println(F("      document.getElementById('radioValue').textContent = data.radio;"));
  client.println(F("      document.getElementById('magneticValue').textContent = data.magnetic;"));
  client.println(F("    })"));
